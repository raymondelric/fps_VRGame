...
public class RocketLauncher : MonoBehaviour
{
    ...
    public void shootFunc(SocketIOEvent e) {
        Debug.Log("I'm at shootFunc");
        Debug.Log("[SocketIO] Shoot received: " + e.name + " " + e.data);
        shootNow = true;
    }

    public void reloadFunc(SocketIOEvent e) {
        Debug.Log("I'm at reloadFunc");
        Debug.Log("[SocketIO] Reload received: " + e.name + " " + e.data);
        reloadNow = true;
    }

    void Start()
    {
        GameObject go1 = GameObject.Find("SocketIO");
        socket = go1.GetComponent<SocketIOComponent>();
        socket.On("shoot", shootFunc);
        socket.On("reload", reloadFunc);
    ...
    void Update()
    {
		...
		if (shootNow & curAmmo > 0 & _rateofSpeed > RateOfSpeed & rpgmode == 0)//if shoot
        {
            Shot();
        }

		if (reloadNow & rpgmode < 4 & inventoryAmmo > 0 & curAmmo != maxAmmo)// if reload
        {
